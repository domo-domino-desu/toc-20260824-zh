...
True, lots of functions are described by a formula
...
\subsection*{Pictures}
We can illustrate functions with the familiar $xy$~axes
\begin{center}
...
Below on the left is the action of the exclusive~or Boolean function.
On the right is the
...
$f(S)=\set{0,1,4}$.
Under the floor function given by $g(x)=\floor{x}$,
...
and the function associates each input with the resulting output.
...
but those numbers are not in the domain of the computed function.

Consequently we will use terminology that differs somewhat from 
the definition given above.  
When describing a computed function 
...
but its connotation is sometimes helpful.)
...
the usual formula for the sum of two fractions. 
...
Following the usual formula with the unhatted pairs gives 
...
and the earlier equalities give that these are equal. 

One more note about well-definedness:~if a function is 
suitable for graphing on $xy$~axes, $\map{f}{\R}{\R}$, then 
