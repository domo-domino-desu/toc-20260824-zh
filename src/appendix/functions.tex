...
\subsection*{Partial functions}\index{partial function}\index{function!partial}
We often consider the function computed by a machine.
We might have a machine that inputs and outputs
natural numbers (or one that does bit strings, etc.)\ 
and the function associates inputs with the resulting outputs.
However, many machines have inputs for which there is no 
associated output.
Such a machine accepts all natural numbers so in a sense its domain is~$\N$, 
but input numbers with no output are not in the domain of the computed function.

Consequently when describing a computed function
we will use terminology that differs from 
the definition given above.
We often fix a convenient set of inputs~$D$, such as the set of all 
strings~$\kleenestar{\Sigma}$ or that of all natural numbers~$\N$,
and refer to that set as the domain.
To emphasize that the function may be undefined on some of its elements,
we say that it is a 
...
The definition of a function includes the requirements 
that 
each domain element~$x$ is associated with at least one codomain 
element~$y$, and also that
each such~$x$ is associated with at most one such~$y$.
% \begin{parenum}
...
Often we need to verify well-definedness 
when defining a function on sets via a function on their elements.\footnote{%
...
To illustrate, here we use the collection of fractions, 
taken as ordered pairs $(n,d)$ where $d\neq 0$, 
and grouped together into sets~$X$ of pairs that are equivalent, 
so that the group containing $(1,2)$ also has $(2,4)$ and $(3,6)$.

First, a failure.
We could try to define an operation that inputs two sets 
...
because they are not equivalent fractions.

Now for a function.
Again there are two inputs $X_0$ and~$X_1$.
...
suitable for graphing on $xy$~axes then 
