...
The standard formal definition fixes two sets, 
...
The function~$d$ is $2$-ary, while some other
...
doesn't follow a prescribed conduct any more sensible than an
...
concise and intelligible behavior pattern, 
...
\subsection*{Pictures}
We often illustrate functions with the familiar $xy$~axes.
\begin{center}
...
\subsection*{Partial functions}\index{domain|for computable functions}
...
and the function associates each input with the machine's resulting output.
...
Consequently we will use a convention that differs somewhat from 
...
To raise the point that the function may be undefined on some of its elements,
we refer to it as a 
...
but we use it to connote that this map may be 
not defined on some $d\in D$.)
...
To verify that a relationship is a function 
usually it is~\zcref{it:AtMostOne} that we must check (if the relationship
is partial then the other item is not a concern).\footnote{%
...
We often run into the need to check this when working with sets. 
So for examples we use the fractions, taken as ordered pairs $(n,d)$ 
(where $d\neq 0$).
We group together fractions that are equivalent, so that $(1,2)$ is in the 
same group as $(2,4)$ and $(3,6)$.

First, an association that fails to be a function.
We could try to define a sum of the groups $X_0$ and~$X_1$ by taking a pair from 
each $(n_0,d_0)\in X_0$ and  $(n_1,d_1)\in X_1$
and making the result be the group containing $(n_0+n_1,d_0+d_1)$. 
However, let $X_0$ be the group containing $(1,2)$
and let $X_1$ be the group containing $(3,4)$ as well as $(-3,-4)$.
Then adding the numerators and denominators gives $(4,6)$ and~$(-2,-2)$. 
However
those two are not in the same group because they are not equivalent fractions.
So the proposed sum of $X_0$ and~$x_1$ is not well-defined, 
because it does not give a unique outcome.

Now instead define the sum of $X_0$ and~$X_1$ by taking a pair from 
each $(n_0,d_0)\in X_0$ and  $(n_1,d_1)\in X_1$
and making the result be the group containing $(n_0d_1+n_1d_0,d_0d_1)$,
the familiar formula for addition of two fractions. 
Then we can verify that no matter which pair we take from $X_0$ and which
pair from~$X_1$, the resulting combination will be a pair in the same
outcome group.
Briefly, fix $(n_0,d_0),(\hat{n}_0,\hat{d}_0)\in X_0$
and $(n_1,d_1),(\hat{n}_1,\hat{d}_1)\in X_1$
Then cross multiplying the fractions gives $n_0\hat{d}_0=\hat{n}_0d_0$
and $n_0\hat{d}_0=\hat{n}_0d_0$.
Combining the unhatted ones gives $(n_0d_1+n_1d_0,d_0d_1)$
while the hatted ones give 
$(\hat{n}_0\hat{d}_1+\hat{n}_1\hat{d}_0,\hat{d}_0\hat{d}_1)$.
Using the above cross multiplication equalities will show that 
the two cross products here are also equal.

% As an example of a relationship that fails, consider the set of ordered pairs
% $(x,y)$ where $y^2=x$. 
% This relationship is not functional\Dash it is not well-defined\Dash because
% $x=9$ is associated with both $y=3$ and~$y=-3$.
% In contrast, for the set of ordered pairs
% $(x,y)$ where $x^2=y$, verify~\zcref{it:AtMostOne} by supposing that
% both $y_0=x^2$ and $y_1=x^2$ and noting that then $y_0=y_1$. 

One more note:~in the case of a function that is suitable for graphing 
on $xy$~axes, $\map{f}{\R}{\R}$, 
