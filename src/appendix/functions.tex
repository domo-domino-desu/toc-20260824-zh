...
that every~$x\in D$ appears in one and only one pair (more on this is below).
We write $f(x)=y$ 
...
An important point is that a function isn't a 
formula or rule.
...
We say that $\operatorname{dist}$ is $2$-ary, while some other
...
as with $x=(3,5)$, then we often drop the parentheses, so that
...
For instance, the range of the floor function shown above
...
$f(x)=\smash{\sqrt{2x^4+2x^2+15}}$, we would usually be content to note 
...
understand which inputs are an issue.
...
\subsection*{In our subject}\index{domain!in the Theory of Computation}
The Theory of Computation sometimes uses these terms in a way that is related
to what's above
but also is in conflict with it.
Often when we study a function~$f$ we will
fix a convenient set of inputs~$D$, such as the set of all 
strings $\kleenestar{\Sigma}$ or the natural numbers~$\N$,
and refer to $D$ as the function's domain although it may be that $f$
is undefined on some of $D$'s elements.
In this case 
we say that~$f$ is a 
...
Strictly speaking, in this terminology the `partial' is redundant
since any function is partial, including a total function.
But often `partial' connotes 
that $f$ is not defined for one or more $d\in D$.
...
because $x=3$ does not have one and only one associated~$y$.
...
there could easily be more than one, say, \str{mdouglas},
making the relation
$(\text{email},\text{person})$ be not 
well-defined.
