...
input value has a unique associated output value.
...
$a_nx^n+\cdots+a_1x+a_0$ and a Boolean value \true{} or~\false,
...
every~$x\in D$ appears in one and only one pair.
We write $f(x)=y$ 
or $x\mapsto y$. 
(Note the different characters used for the arrow in $\map{f}{D}{C}$ and the one
in $x\mapsto y$). 
...
Likewise for a formula that returns World Series winners,
...
\end{center}
On the right is a vertical variant of the bean diagram, showing the
