...
For the precise definition, fix two sets, 
...
that every~$x\in D$ appears in one and only one pair (more on this below).
We write $f(x)=y$ 
or $x\mapsto y$ and say `$x$ maps to~$y$'. 
(Note the difference between the arrow symbols in $\map{f}{D}{C}$ and 
$x\mapsto y$). 
...
An important point is what a function isn't:~it isn't a 
formula or rule.
The function that gives the US presidents, 
$f(0)=\text{George Washington}$, etc., has no sensible formula
and isn't determined by any rule less complex than an
exhaustive listing of cases.
The same holds for a function that returns winners of the US World Series,
including next year's winner.  
True, 
...
there is one and only one associated output.
If we can calculate the outputs from the inputs, that's great,
but that is not required.

Some functions take more than one input, for instance 
$\operatorname{dist}(x,y)=\sqrt{x^2+y^2}$.
We say that $\operatorname{dist}$ is $2$-ary, and other
...
\begin{center}
  ...
  \hspace{2em plus 0.5fil}
  \vcenteredhbox{\includegraphics{appendix/asy/functions005.pdf}}
\end{center}
On the right is a variant of the bean diagram, using the number line to
show the
...
The image of the entire domain is the function's
\definend{range},\index{range of a function}\index{function!range}
$\operatorname{ran}(f)=f(D)=\set{f(d)\suchthat d\in D}$.
For instance, the range of the floor function 
is the set of integers.

Note the difference between the range and the 
codomain;~the\index{codomain versus range}
codomain is a convenient superset.
An example is that 
for the function with real inputs 
$f(x)=\smash{\sqrt{2x^4+2x^2+15}}$ we would often be content to note 
that the polynomial is always nonnegative and so the output is real, 
writing $\map{f}{\R}{\R}$, 
rather than troubling with its exact range. 
...
Sometimes the function's domain needs attention.
...
However, we are often casual about this.

In particular, in this subject we often have a function $\map{f}{\N}{\N}$
such that on some elements of the domain the function
is undefined. 
We say that~$f$ is a \label{def:PartialFunction} 
\definend{partial function}.\index{partial function}\index{function!partial} 
If instead it is defined on all inputs then 
it is a \definend{total function}.\index{total function}\index{function!total}

We sometimes have a function $\map{f}{D}{C}$
and want to cut the domain back to some subset $S\subseteq D$.
The 
...
We refer to this condition by saying that functions are \definend{well-defined}.

When we are considering a relationship 
between $x$'s and $y$'s and
asking if it is a function, well-definedness is typically what is at 
issue.\footnote{%
...
first initial and last name, but the problem is that 
there can easily be more than one, say, \str{mdouglas}.
That is, the relation
$(\text{email},\text{person})$ could be not 
well-defined.
...
the vertical line
at~$x$ intercepts $f$'s graph in one and only one point.
...
\item
  ...
  Before the proof we give an illustration, which may serve
  as a hint.
