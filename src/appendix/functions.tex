...
and isn't determined by any rule less complex than a listing of separate cases.
...
As well, many functions are computed by a program.
...
If you can calculate the outputs, that's great,
but that is not required to be a function.
...
Under the squaring function the image of $S=\set{0,1,2}$ is 
$f(S)=\set{0,1,4}$.
...
$\map{g}{\R}{\R}$ given by $g(x)=\floor{x}$ is the integers.
...
$f(x)=x^6-31x^5+13x^2+4$ we would usually write 
$\map{f}{\R}{\R}$,
taking the codomain to be all of the reals, 
rather than trouble with its exact range. 
...
because there is not one and only one~$y$ for each~$x$.
...
codomain bean, the bean on the right, 
there is at most one arrow ending at~$y$.
\begin{center}
  \vcenteredhbox{\includegraphics{appendix/asy/functions004.pdf}}
\end{center}
The most common way to verify that a function~$f$ is one-to-one is to 
...
we can conclude some things about the number of elements in the sets.
The first is that the number of elements in the domain is less than or equal 
to the number in the codomain.
The other is that if the function is onto then
...
and the one on the right shows a correspondence between the natural numbers
and the primes.
\begin{center}
...
As the picture on the left above suggests, where
...
$\composed{f}{g}=\sin(x^2)$ is different from 
$\composed{f}{g}=(\sin x)^2$, since they are unequal when $x=\pi$.
...
and let $\map{f}{\R^2}{\R^3}$ be the map
$(x,y)\mapsto(x,y,0)$.
