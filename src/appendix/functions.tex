...
depending on whether~$34$ is a root of that polynomial.
And still another is the association of a year since 1903 with the
name of that year's winner of the US baseball championship.

The formal definition fixes two sets, 
...
\definend{well-defined},\index{well-defined function}\index{function!well-defined}
that every~$x\in D$ appears as the first entry 
in one and only one pair. 
%</def:functioni>
(More on this is below.)
%<*def:functionii>
We denote the association with \definend{$f(x)=y$} 
...
%<*def:functioniii>
Some functions such as $d(x_0,x_1)=x_0^2+x_1^2$
take multiple inputs (or we can take the input to be a pair).
This function~$d$ is $2$-ary while other
functions are $3$-ary, etc.
The number of inputs is the function's \definend{arity}.
A function of arity~$0$, one that takes no inputs, is constant.
...
is a new function, with
domain~$S$ and codomain~$C$, defined by $\restrictionmap{f}{S}(s)$ is $f(s)$ 
for~$s\in S$.   
%</def:functioniii>

Finally, a comment on what a function is not:
\citetext{many introductions describe a function as a `rule'}{%
  For examples of sources, including many that are quite reputable, enter a 
  `a function is a rule' in a search engine.}.
That's wrong.
The function that associates a year with its baseball champion
doesn't follow any prescribed conduct more sensible than brute enumeration.
True, many functions are described by a formula
and as well many are computed by a program.
But what makes something a function is not the existence of a 
concise pattern of determined behavior, 
it is that for each input there is one and only one output.
In particular, a key result in this book is that there
are functions that are not computable, and
consequently we often study input-output behaviors for which there is no `rule'
...
\subsection*{Pictures}
%<*disc:Picturesi>
We often illustrate functions by using the familiar $xy$~axes.
\begin{center}
...
%</disc:Picturesi>
%<*disc:Picturesii>
But we sometimes instead use a bean diagram,\index{bean diagram} 
which separates the domain and codomain.
Below on the left is the action of the Boolean function~XOR.\index{exclusive or@exclusive or \textsc{XOR}}\index{XOR@\textsc{XOR}, exclusive or}\index{Propositional logic!exclusive or@exclusive or \textsc{XOR}}
On the right is the
absolute value function,
using number lines in place of beans.
\begin{center}
...
$f(S)=\set{0,1,4}$.
Under the floor function $g(x)=\floor{x}$
the image of the nonnegative reals is
...
second is a convenient superset of the first.
As an example, for the function
$f(n)=2n^4-2n^2+18$ we may be content to note 
that an integer input will produce an integer output 
and just write $\map{f}{\Z}{\Z}$,
rather than troubling to find which numbers are and are not elements
of the range. 



\subsection*{Functions that are partial}\index{partial function}\index{function!partial}
%<*def:PartialFunctions>
We often fix a machine and consider its input/output behavior.
This is the function computed by the machine.
It associates bitstring inputs with bitstring outputs
(or we might interpret them as natural numbers, etc.)

However,
machines may have inputs for which there is no associated output,
while the above definition of a function requires that for every $x$
there must be a~$y$.
To smooth over this discrepancy, the convention is that when studying these
functions we fix a convenient set of inputs, such as the set of all 
bitstrings~$\kleenestar{\B}$ (or the set of natural numbers~$\N$, etc.),\ 
and refer to it as the function's \definend{natural domain}.\index{natural domain}\index{function!natural domain}\index{partial function!natural domain}
We also overload the functional notation and write such things as 
$\map{f}{\kleenestar{\B}}{\kleenestar{\B}}$\!.
Then to acknowledge that~$f$ may be undefined on some of this set's elements,
so that its actual domain is smaller, 
we say that it is a 
\definend{partial function}.\label{def:PartialFunction} 
If we know that the function is defined on all inputs then 
...
but its connotation can be helpful.)
...
%<*def:WellDefinedi>
The definition of a function includes the requirement
of well-definedness, 
that each domain element~$x$ is associated with at least one codomain 
element~$y$, and also that each such~$x$ is associated with at most one~$y$.
%</def:WellDefinedi>

%<*def:WellDefinedii>
Often we need to verify this property  
when we define a function on sets via a function on their elements.\footnote{%
  Sometimes people say that they are, ``checking that the function is 
  well-defined.''
  This may appear confused because   
  if it is a function then it is by definition well-defined.
  However, natural language is funny this way\Dash while tigers must have 
  stripes, we may nonetheless still say ``striped tiger.''}
To illustrate, we will use the fractions, 
taken as the sets~$X$ of ordered pairs $(n,d)$ (where $d\neq 0$) 
where two pairs $(n_0,d_0)$ and $(n_1,d_1)$ are in the same~$X$ 
if they are equivalent fractions, 
so that the set containing $(1,2)$ also contains $(2,4)$ and $(3,6)$.
%</def:WellDefinedii>

%<*def:WellDefinediii>
First, a failure.
We could try to define an operation on two input sets 
$X_0$ and~$X_1$ by taking a pair from each, 
$(n_0,d_0)\in X_0$ and  $(n_1,d_1)\in X_1$,
and giving the output set~$X_2$ as the one containing $(n_0+n_1,d_0+d_1)$. 
This is not well-defined.
For an instance of the difficulty, take $X_0$ to be the set containing $(1,2)$
and $X_1$ to be the one containing $(3,4)$ as well as $(-3,-4)$.
Then adding the numerators and denominators gives $(1,2)+(3,4)=(4,6)$ 
and $(1,2)+(-3,-4)=(-2,-2)$. 
But no~$X_2$ contains both $(4,6)$ and $(-2,-2)$
because these are not equivalent.
%</def:WellDefinediii>

%<*def:WellDefinediv>
Now for a success.
Again we start with two inputs $X_0$ and~$X_1$.
Again we take a pair from each,
$(n_0,d_0)\in X_0$ and  $(n_1,d_1)\in X_1$, but here
we make the result~$X_2$ be the set containing $(n_0d_1+n_1d_0,d_0d_1)$,
the usual common denominator formula for the sum of two fractions. 
To verify that this operation on the $X$'s is well-defined 
we fix $X_0$ and~$X_1$ and show that
although combining different member pairs from these two 
yields many different resulting pairs, all of them are in the same output set. 
That argument goes:~take two pairs from each group, 
$(n_0,d_0),(\hat{n}_0,\hat{d}_0)\in X_0$
and $(n_1,d_1),(\hat{n}_1,\hat{d}_1)\in X_1$.
Because they are equivalent fractions, cross multiplying gives 
$n_0\hat{d}_0=\hat{n}_0d_0$ and $n_1\hat{d}_1=\hat{n}_1d_1$.
Following the common denominator formula with the unhatted pairs gives 
$(n_0d_1+n_1d_0,d_0d_1)$
while the hatted pairs give 
$(\hat{n}_0\hat{d}_1+\hat{n}_1\hat{d}_0,\hat{d}_0\hat{d}_1)$.
These two are equivalent fractions:~the 
cross products are 
$\hat{d}_0\hat{d}_1\cdot(n_0d_1+n_1d_0)
=n_0\hat{d}_0\cdot d_1\hat{d}_1+n_1\hat{d}_1\cdot d_0\hat{d}_0$
and $d_0d_1\cdot(\hat{n}_0\hat{d}_1+\hat{n}_1\hat{d}_0)
=\hat{n}_0d_0\cdot d_1\hat{d}_1+\hat{n}_1d_1\cdot d_0\hat{d}_0$,
and the earlier equalities show that these are equal. 
%</def:WellDefinediv>

%<*def:WellDefinedv>
One more note about well-definedness:~if a function is 
suitable for graphing on the $xy$~axes then 
visual proof of well-definedness is that for any $x$ in the domain, 
the vertical line through~$x$ intercepts $f$'s graph in exactly one point.
%</def:WellDefinedv>
...
$(x,y)\in f$\!, it 
requires that each~$x$ be in one pair and no more than one pair,
but it does not require the same of the $y$'s.

%<*def:OneToOne>
A function is 
\definend{one-to-one},\index{one-to-one function}\index{function!one-to-one} 
or \definend{$1$-$1$} or
an \definend{injection},\index{injection function}\index{function!injection} 
...
If a function is suitable for graphing on the $xy$~axes then 
...
The function below is onto
because every element in the codomain bean
...
start with a generic codomain element~$y$ and then 
...
Where the function can be graphed on $xy$~axes,  
...
for one, they are unequal when $x=\pi$.
%</def:Compositioni>
...
$g$~is a \definend{left inverse}\index{left inverse function}\index{function!left inverse}\index{inverse of a function!left} 
function of~$f\!$, or what is the
same thing, $f$ is a 
\definend{right inverse}\index{right inverse function}\index{function!right inverse}\index{inverse of a function!right}  
of~$g$.
...
\begin{exparts}
\item
  The number of elements in the range is less than or equal 
  to the number in the domain, $|\range(f)|\leq |D|$.
\item
