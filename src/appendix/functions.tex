...
in one and only one pair (more on this below).
...
This function~$d$ is $2$-ary, while some other
functions are $3$-ary, etc.;~the
number of inputs is the function's \definend{arity}.
...
Finally, what a function is not:
\citetext{many introductions say that a function is a `rule'}{%
  For examples of sources, including many that are quite reputable, enter a 
  `a function is a rule' in a search engine.}.
That's wrong.
The function that associates a year with its baseball winner 
doesn't follow any prescribed conduct more sensible than an
exhaustive enumeration.
True, many functions are described by a formula
and as well many are computed by a program.
But what makes something a function is not the existence of any 
concise and intelligible pattern of behavior, 
it is that for each input there is exactly one associated output.
In particular, a key result in this subject is that there
are functions that are not computable and
so we often work with input-output behaviors for which there is no `rule'
in any ordinary sense of that term.



\subsection*{Pictures}
We can draw functions by using the familiar $xy$~axes
\begin{center}
...
Below on the left is the action of the exclusive~or function.
On the right is the
...
when defining an operation on sets via an operation on its elements.\footnote{%
...
So for the examples here we use the fractions, taken as ordered pairs $(n,d)$ 
where $d\neq 0$, 
and grouped together into sets of ones that are equivalent, 
so that $(1,2)$ is in the same group as $(2,4)$ and $(3,6)$.

First is an association that fails to be a function.
We could try to define a map that inputs two sets 
$X_0$ and~$X_1$, takes a pair from each, 
$(n_0,d_0)\in X_0$ and  $(n_1,d_1)\in X_1$,
and finds the output set~$X_2$ as the one containing $(n_0+n_1,d_0+d_1)$. 
An instance of the difficulty is if $X_0$ contains $(1,2)$
and $X_1$ contains $(3,4)$ as well as $(-3,-4)$.
Then adding the numerators and denominators gives $(4,6)$ and~$(-2,-2)$. 
but no~$X_2$ contains both of these
because they are not equivalent fractions.
So the described association is not well-defined.

Now for an association that is a function.
...
$(x,y)$, it 
requires that each~$x$ be in one pair and only one pair
...
is defined by $\iota(x)=x$ when the 
