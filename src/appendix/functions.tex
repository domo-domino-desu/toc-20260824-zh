...
We denote the association as \definend{$f(x)=y$} 
...
% (We formalize this as that the domain elements $d\in D$ 
% are pairs or triples, etc.) 
...
$f(S)=\set{0,1,4}$.
And under the floor function $g(x)=\floor{x}$
...
% \begin{parenum}
%   \item each domain element~$x$ is associated with at least one codomain 
%    element~$y$, and
%   \item \label{it:AtMostOne}each such~$x$ is associated with at most one such~$y$.
% \end{parenum}
