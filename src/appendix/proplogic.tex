...
right-hand table.
In the $P\wedge Q$ column the third row, the \true~\false{} row, 
shows a value of~$F\!$.
...
The three operators `$\neg$', `$\wedge$', and `$\vee$' 
form a complete set in that, using only these three, 
we can reverse the activity above:~given any  
truth table, with these three we can produce an expression 
giving that table. 
...
Intuition demands that there be a normal form that is based on 
the $\false$ rows.
We introduce it by illustrating with $E_0$.
...
yielding
$E_0\equiv (P\vee \neg Q)\wedge(\neg P\vee Q)\wedge(\neg P\vee \neg Q)$.
\definend{Conjunctive Normal form}\index{Conjunctive Normal form CNF}\index{Propositional logic!Conjunctive Normal form}
or \definend{CNF}
is a conjunction of clauses, where each clause is a disjunction of literals.

There is a quicker way to construct a Boolean expression in CNF from a 
truth table. 
For illustration start with $E_1$. 
Make a clause for each \false{} line
and join them with $\wedge$'s.
Then fill in the clauses by:~for each of the truth table's 
associated $\false$ lines, 
where the variable $X$ is assigned \true{} we write $\neg X$
and where $X$ is assigned~\false{} we write the literal as $X$.
\begin{align*}
  E_1
  &\equiv(\quad)\wedge(\quad)\wedge(\quad)\wedge(\quad)\wedge(\quad)  \\
  &\equiv(P\vee Q\vee R)
    \wedge(P\vee \neg Q\vee \neg R)
    \wedge(\neg P\vee Q\vee \neg R)       \\
    &\qquad \wedge(\neg P\vee \neg Q\vee R)
    \wedge(\neg P\vee \neg Q\vee \neg R)
\end{align*}
For example, in $E_1$'s first \false{} line all three variables have been
assigned~\false{} so we fill the clause with $P\vee Q\vee R$.
The second of $E_1$'s \false{} lines has $P$ assigned~\false{} but $Q$ and~$R$
have been assigned~\true, and so we fill in the second clause with 
$P\vee \neg Q\vee \neg R$. 
...
has inputs that are Boolean, that is, they are either $T$ or~$F$, 
