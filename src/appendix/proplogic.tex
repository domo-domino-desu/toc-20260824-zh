...
We can operate on propositions, including taking the
...
and \definend{or} (also \definend{disjunction}).\index{logical operator!or}\index{disjunction}
...
same final column. 
We write, for instance, $P\rightarrow Q\equiv\neg P\vee Q$
...
truth table we can use the three to produce an expression having that table. 
...
so for $E_0$ the expression $\neg P\wedge \neg Q$ works. 
For $E_1$ we must do all of the $\true$ rows.
Target the second row with $\neg P\wedge \neg Q\wedge R$.
Target the third row with $\neg P\wedge Q\wedge \neg R$, and
for the fifth use~$P\wedge\neg Q\wedge\neg R$.
Get $E_1$ by joining these clauses with $\vee$'s.
\begin{equation*}
(\neg P\wedge \neg Q\wedge R)
  \vee(\neg P\wedge Q\wedge \neg R)
  \vee(P\wedge\neg Q\wedge\neg R)
...
Intuition requires that there be an approach that starts 
with the $\false$ rows.
