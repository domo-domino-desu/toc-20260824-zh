...
the \citetext{\definend{empty string}, denoted~$\emptystring$}{Possibly 
  ...
  `empty', \textit{leer}.  \autocite{csse:Siren}}.\index{string!empty}\index{empty string@empty string, $\emptystring$}
%</def:String>
...
A convenience of this form is that 
...
%<*dis:OmitCommas>
For small-scale examples and exercises, we use the shortcut of 
working with
alphabets of single-character symbols and then writing strings
...
Strings over this alphabet are  \definend{bitstrings}\index{bitstring} 
...
appends the second sequence to the 
first:~$\sigma\concat\tau=\sequence{s_0\ldots s_{n-1},t_0,\ldots t_{m-1}}$.
\label{def:substring}
Where
$\sigma=\tau_0\concat \cdots \concat\tau_{k-1}$ then we say that
...
%<*def:Palindrome>
A string that equals its own reversal is a 
\definend{palindrome};~examples\index{palindrome}
