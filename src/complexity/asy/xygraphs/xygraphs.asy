// xygraphs.asy
//  Graphs of fcns

import settings;
settings.outformat="pdf";
settings.render=0;

unitsize(1.25pt);

// Set LaTeX defaults
import settexpreamble;
settexpreamble();
// Asy defaults
import jh;

string OUTPUT_FN = "xygraphs%02d";

import graph;
// some parameters
real axis_arrow_size = 0.35mm;
real axis_tick_size = 1mm;




// ============== compare number primes less than n with lg n ================
picture pic;
int picnum = 0;
size(pic,6.5cm);
// real scalefactor = 1000;
// scale(pic,Linear(scalefactor),Linear(5000));
scale(pic,Linear(1),Linear(5));

// limits
real xmin=1;  // lg(xmin)=0
real xmax=1000000;
real ymin=0;
real ymax=100000;

// fcns
real f(real x) {return x/log(x);}
real g(real x) {return 10*log(x)/log(2);}  // they left out log2

// curves
path f=graph(pic,f,xmin+1,xmax,n=400);
draw(pic,f,MAINPEN+linewidth(0.4pt)+highlightcolor);
label(pic,"$x/\ln(x)$",Scale(pic,(900000,70000)),SE,TICLABELPEN+highlightcolor);

// draw the dots
pair [] pts = {(100,25),
               (1000,168),
               (5000,669),
               (10000,1229),
               (50000,5133),
               (100000,9592),
               (200000,17984),
               (300000,25977),
               (400000,33860),
               // (250000,22044),
               (500000,41538),
               (600000,49098),
               (700000,56543),
               // (750000,60238),
               (800000,63951),
               (900000,71274),
               (1000000,78498)
};

dotfactor=1.5; // http://asymptote.sourceforge.net/FAQ/section3.html
for(pair pt: pts) {
   dot(pic, Scale(pic,pt), FCNPEN_SOLID, Fill(FCNPEN_SOLID));
}
label(pic,"$(1\,000\,000,78\,498)$",Scale(pic,(1000000,78498)),N,TICLABELPEN);

// axes
xaxis(pic,YZero,
      xmin-1, xmax+75000,
      RightTicks(Label("%",TICLABELPEN), Step=500000, step=100000,
		 beginlabel=false, endlabel=true,
		 Size=axis_tick_size, size=0.65*axis_tick_size,
		 extend=false, begin=false),
      p=AXISPEN,
      Arrow(TeXHead,axis_arrow_size));
label(pic,"$1\,000\,000$",Scale(pic,(1000000,0)),1.5*S,TICLABELPEN);
label(pic,"$500\,000$",Scale(pic,(500000,0)),1.5*S,TICLABELPEN);
yaxis(pic,XZero,
      ymin, ymax+10000-1,
      LeftTicks(Label("%",TICLABELPEN), Step=50000, step=10000,
		beginlabel=false, endlabel=true,
		Size=axis_tick_size, size=0.65*axis_tick_size,
		extend=false, begin=false),
      p=AXISPEN,
      Arrow(TeXHead,axis_arrow_size));
label(pic,"$100\,000$",Scale(pic,(0,100000)),1.5*W,TICLABELPEN);
label(pic,"$50\,000$",Scale(pic,(0,50000)),1.5*W,TICLABELPEN);

shipout(format(OUTPUT_FN,picnum),pic,format="pdf");

// sage: for n in [100, 1000, 5000, 10000, 50000, 100000, 200000, 300000, 400000, 5
// ....: 00000, 600000, 700000, 800000, 900000, 1000000]:
// ....:     print(n, ", ", pari(n).primepi())
// ....: 
// ....: 
// 100 ,  25
// 1000 ,  168
// 5000 ,  669
// 10000 ,  1229
// 50000 ,  5133
// 100000 ,  9592
// 200000 ,  17984
// 300000 ,  25997
// 400000 ,  33860
// 500000 ,  41538
// 600000 ,  49098
// 700000 ,  56543
// 800000 ,  63951
// 900000 ,  71274
// 1000000 ,  78498






