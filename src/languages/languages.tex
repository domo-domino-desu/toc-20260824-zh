...
\begin{answer}
  ...
  \item 考虑集合
    ...
    Cartesian积是 $n$ 元组的集合，而 $\Sigma^n$ 是 $\kleenestar{\Sigma}$ 的一个子集）。
    它是可数多个可数集的Cartesian积，因此根据第二章的
    \zcref{cor:CrossProdCountablyManyIsCountable}，它是可数的。
    所以存在单射且满射的函数 $\map{g}{\N}{S}$。
...
如果存在一个单射且满射的映射 $\map{f}{\mathcal{N}}{\hat{\mathcal{N}}}$，
