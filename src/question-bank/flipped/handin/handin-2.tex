...
\makebox[\textwidth]{\sc MA 208, Theory of Computation\hfill Weekly Hand-in\hfill Due: 2021-Feb-12}
\vspace*{1ex}
\begin{center}
...
\begin{solution}
Consider the real number function
given by $f(x)=(8/3)\cdot(4-x)-1$.
% Observe that $f(4)=-1$ and that $f(1)=7$, and that because the function is
% continuous, we have $\map{f}{\rightclosed{1}{4}}{\leftclosed{-1}{7}}$.

To verify that the function is one-to-one, suppose that $f(x_0)=f(x_1)$.
Then $(8/3)\cdot(4-x_0)-1=(8/3)\cdot(4-x)-1$.
