...
(define algol-to-scm-tests
  ...
  (test-suite
   ...
   (test-case
    ...
    (clear-regs!)
    (output 3)
    (let ([pgm (read-loop-pgm "rotate-shift-right.loop")])
      (let ([data '(1 2 3 4)])
        (check-equal? (loop-without-parens pgm data) "3 1 2"))
      )
    (clear-regs!)
    (output 2)
    (let ([pgm (read-loop-pgm "swap-two.loop")])
      (let ([data '(1 2)])
        (check-equal? (loop-without-parens pgm data) "2 1"))
      )
    (clear-regs!)
    (output 3)
    (let ([pgm (read-loop-pgm "rotate-shift-left.loop")])
      (let ([data '(1 2 3)])
        (check-equal? (loop-without-parens pgm data) "2 3 1"))
      )
    (clear-regs!)
    (output 3)
    (let ([pgm (read-loop-pgm "concatenate-rotations.loop")])
      (let ([data '(1 2 3)])
        (check-equal? (loop-without-parens pgm data) "1 2 3"))
      )
    ) ;; close test-case
   ) ;; close suite
  ) ;; close fcn
