...
\pause
(1)~Words starting with \str{a}.
\pause
(2)~Words containing \str{bb}.
\pause
(3)~Words ending in \str{ing}.
\pause
(4)~Words containing a \str{t}.  % which is more: t or e?
\pause
(5)~Words in which a \str{w} follows a \str{k}.
\pause
(6)~Words in which one \str{k} preceeds one \str{w}.
\pause
(7)~Words of exactly five letters.
\pause
(8)~Words of at least five letters.
\pause
(9)~Words of at least five letters with at least two vowels.
\pause
(10)~Words in which all five vowels occur, in ascending order.
\end{example}
\end{frame}
