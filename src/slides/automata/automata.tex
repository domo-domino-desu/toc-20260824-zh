...
\end{center}
Our default convention is that that the start state is $q_0$.
\end{frame}
...
\end{center}
(It is a gotcha question; all strings begin with zero copies of \str{HEF}.)
\end{example}
\pause
\begin{example}
...
deterministic\Dash no pair of instructions can begin with 
the same $q_pT_p$.
...
neverending expansions, at a necessarily-infinite process.
But we are not interested in that.
We simply define that a string~$\sigma$ is derivable if there is at least one
successful sequence of $\derives$ steps, and then we think about the set
...
There is a second way to proceed, the way that we actually do these
problems.
We intuit a sequence of steps and then verify our guess.   
\end{itemize}
...
In truth though, 
how we actually handle this is to look forward, hoping to guess a 
...
This machine has two arrows labeled~\str{0} that are leaving $q_0$.
Also, out of $q_1$ comes no arrow with a~\str{1}.
\begin{center}\small
...
Above, we recalled the problem where we start with a grammar and then
we are given a string and want to know if it is derivable.
The obvious approach is 
a breadth-first traversal of the tree of all derivations.

% \pause
Another illustration is the Travelling Salesman problem.
We look for an efficient trip that visits every state capitol in the 
...
This obviates the paradox of guessing, at least for Finite State machines.
\end{frame}
...
\pause
\begin{example}
This machine~$\hat{\FSM}$ with alphabet $\Sigma=\set{\str{0},\str{0}}$ 
accepts an input string only
