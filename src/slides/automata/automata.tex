...
We will 
do it here for Finite State machines and we will do it in the next chapter
for Turing machines.
% We will see two ways to informally think of nondeterminism.
% We will also see examples of nondeterminism, and its uses.
% In particular, we
...
The machine model that we are introducing 
sometimes leaves the initial impression of being odd.
So before the definition we will do some preliminary exposition. 
We will first note that we already have some experience with
such computations.
Then we will see an example of a nondeterministic Finite State machine
and trace through the acceptance of a string,
which will give us two mental models for how to think about these computations.
The full definition comes after that.
\end{frame}


\begin{frame}{Recall: grammars}
Deriving a string from a grammar is a kind of computation.
It is symbol pushing.

For example, from this grammar
\begin{grammar}
  S \produces{} T\trm{b}U
...
\begin{frame}
Two points about this kind of computation
bear on the definition of nondeterministic Finite State machines.

\begin{enumerate}
\item
In the \mbox{\texttt{T}\produces{}\texttt{aT}} rule,
the recursion might mislead a person into thinking that we are looking at 
neverending expansions, and thus at a necessarily-infinite process.
But we are not interested in neverending expansions.
We simply define that a string~$\sigma$ is derivable if there is at least one
successful finite sequence of $\derives$ steps, and then we think about the set
of derivable strings.

\pause\item
Starting with a target string~$\sigma$, 
one way that we could proceed is by brute force:~apply 
all rules to the start symbol, then all allowed rules to each result,
etc. 
This is a breadth-first search of the $\derives$~sequences.
There is a second way to proceed, the way that we have actually done these
problems in the past:~we intuit a sequence of steps and then verify our guess.
\end{enumerate}

By the second point, we already have some experience with a kind of 
nondeterministic computation.
While in the middle of a derivation-of-a-string computation, 
while doing a sequence of $\derives$~steps,
we often find that more than one rule applies.
We could handle this by making room on the scratch paper and working through
the consequences of both rules, 
and in that sense being in more than one state at a time. 
However, 
how we actually handle this is to look forward and guess a 
finite sequence of legal steps that ends with the desired~$\sigma$.
\end{frame}



\begin{frame}{Example of a nondeterministic machine}\vspace*{-1ex}
Leaving $q_0$, this machine has two different arrows labeled~\str{0}.
Also, out of $q_1$ comes no arrow with a~\str{1}.\vspace*{-1ex}
\begin{center}\small
  \vcenteredhbox{\includegraphics{\asydir nfsm/nfsm00.pdf}}
\end{center}
We will walk through what happens when we give this machine 
the input string~\str{00001}.
\end{frame}
...
\begin{center}\small
  ...
  \only<7-8>{\includegraphics{asy/nfsm08.pdf}}% using onslide causes other figs to not be centered
\end{center}
\onslide<8->{
\begin{itemize}
\item 
...
$q_0$ and~$q_1$.
The same happens for other steps.
\item
...
% Consider a grammar \mbox{S\produces{}\trm{aa}S\syntaxor\trm{aaa}S}.
...
% possible derivation, doing a breadth-first traversal of the tree of all
% derivations.

An example is the Travelling Salesman problem.
Suppose that we look for a cyclic trip that visits every state capitol in the 
forty eight contiguous US~states,
in less than $16\,000$ kilometers.
Start at Montpelier~Vermont. 
...
$126$~kilometers. 
Each child would then fork its own set of children, forty six of them.
...
Another example is that above
we recalled the problem where we start with a grammar and then
we are given a string and asked if it is derivable.
An obvious approach is 
a breadth-first traversal of the tree of all derivations.
% \pause
\end{frame}


\begin{frame}{Mental model number two: guessing}
This nondeterministic machine from earlier
accepts a string if it ends in two~\str{0}'s and a~\str{1}.
\begin{center}\small
  \vcenteredhbox{\includegraphics{\asydir nfsm/nfsm00.pdf}}
\end{center}
With input \str{00001}, the problem the machine faces is:~when should it 
stop going around $q_0$'s loop and start to the right?
The example above has the machine accepting this input.
So it has solved the problem\Dash viewed from the outside,
we could say that the
machine has guessed correctly.

\pause
This mindset is jarring because 
