...
to the index, and a program that goes from the index~$e$ 
...
And, by the earlier result, it is strictly less than.
\end{example}
\end{frame}
...
freeze-$2$-inputs-and-leave-$1$ situations.
...
\begin{center}
  \parbox{0.8\textwidth}{Where $e$ is the index of the Turing machine on the left,
...
computable.
But we used Cantor's Theorem that, at least on the face of it, 
says such a thing exists but doesn't construct that thing. 
The spirit of this subject is that we like to compute things.
So we go through the proof of Cantor's Theorem, to effectivize it.
...
\pause
But wait, it looks like that program gives output that isn't on the list
