...
The word `enumerable' comes from the 
...
There is something quite interesting here.
The machine $\TM_e$ that halts only on input~$e$ has a behavior that,
in some sense, depends on its name. 
But when we defined the numbering, we made a bunch of 
arbitrary choices.
But now we find that, no matter what acceptable numbering we choose,
there must be this name/behavior result.
\end{frame}
