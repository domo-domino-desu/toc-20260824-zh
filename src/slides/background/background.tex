...
\item ``Unsolvable'' means unsolvable by a Turing machine.
  ...
  but it isn't a mechanically computable function\Dash no 
...
\begin{frame}
  \frametitle{Unsolvability via the \HP}
...
$\computablefunction{halts\_on\_one\_checker}$ were
...
\begin{frame}
So we have found a way to turn the question of `does $\TMfcn_x(x)\converges$?'
into the question of `does this halt on input $y=1$?'.
...
That is, clearly we can write the program on the left, so by 
Church's Thesis there is a Turing machine to do it.
That machine has an index, $e$.

The \smn~Theorem gives the family of machines on the right, $\TM_{s(e,x)}$.
We have already observed that $\TMfcn_x(x)\converges$ if and only if 
$\computablefunction{halts\_on\_one\_checker}(s(e,x))=1$,
and so if we could mechanically check whether a machine halts on~$1$
then we could solve the \HP.
We can't solve the \HP{}, so $\computablefunction{halts\_on\_one\_checker}$
...
\item The problem of determining whether a given Turing machine
  halts on input~$1$.
  \begin{equation*}
    \mathcal{I}_1=\set{e\suchthat\text{$\TM_e$ halts on input~$1$}}
  \end{equation*}
\item The problem of determining whether a given Turing machine ever
  outputs a 19.
  \begin{equation*}
    \mathcal{I}_2=\set{e\suchthat\text{there is an~$x$ so that $\TM_e$ halts on $x$ and outputs~$19$}}
  \end{equation*}
\item The problem of determining whether a given Turing machine 
  gives as output the square of its input.
\end{enumerate}
...
computable sets are the only sets that we will
ever know.
Semidecidable sets then appear to be at the limit of our knowledge:~they are 
ones that we can at least half know.
\end{frame}
