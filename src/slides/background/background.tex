...
from $\N$ to $S$ where
$S=\R^+=\set{x\in\R\suchthat x>0}$, or
$S=\leftclosed{0}{1}$.
...
A result that we cover only in the exercises says that if there is a one-to-one
function each way then there is a correspondence.)
\end{example}

\begin{example}
Similarly, we have $|\N|\leq |\R|$.
Here, by the earlier result the less-than relation between these
...
freeze-$2$-inputs-and-leave-$1$-as-is situations.
We write $s_{2,1}$ for the function that is the behavior of that program.
We will usually dispense with the subscripts and
...
computable.
But to prove that we used Cantor's Theorem that, at least on its face, 
says such a function exists but doesn't construct it. 
In this subject we prefer to construct things.
So we go through the proof of Cantor's Theorem and effectivize it.
...
\pause
But wait, it looks like that is a program whose output isn't on the list
...
\begin{frame}
So we have found a way to turn the questions of `does $\TMfcn_x(x)\converges$?'
into questions of `does $\TM_i$ halt on input $y=1$?'.
The issue is whether we can ask the latter question uniformily.
...
Church's Thesis there is a Turing machine to do it.
...
Picture a computable function generating a stream of numbers.
\begin{equation*}
...
(a)~$\set{1,3,5}$,
(b)~$\set{2n\suchthat n\in\N}$,
(c)~$\set{y\suchthat\text{there is an $x$ so that $y=\TMfcn_{19}(x)$}}$,
and (d)~$\set{x\suchthat\TMfcn_{19}(x)\converges}$.
\end{example}
\end{frame}
...
ever know.
With that thinking, semidecidable sets are at the limit of our knowledge:~they 
are ones that we can at least half know, where we can determine membership
but cannot determine nonmembership.
\end{frame}
...
\begin{frame}
\ExecuteMetaData[\catchfilefn]{discussion:WhenDiagonalizationFailsi}
\end{frame}
