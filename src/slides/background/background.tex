...
and 
(d)~if $S=\set{x\in\N\suchthat \text{$x$ is a perfect square}}$ then $|S|=|\N|$.
\end{example}
...
\begin{example}
Let $S=\set{10,11,12}$ and $T=\set{0,1,2,3}$.
...
\begin{example}
Similarly, $|\N|\leq |\R|$ via the natural injection map.
By the earlier result that $\R$ is uncountable, this less-than relation between
...
Thus there is an all-powerful Turing machine,~$\UTM$. 
We can tell it,
``Consider the Turing machine~$\TM_e$. 
Have the same output
behavior as this machine 
would on receiving input~$x$.''
This machine will fail to halt if $\TM_e$ fails to halt on~$x$
and otherwise will halt and yield the correct output.

It is universal in that it can be made to have any desired computable behavior,
...
We next consider freezing some of a machine's inputs.
...
Freezing
one if the arguments gives a family of functions.
...
\alert{parametrized} by the exponent. 
...
can answer.
The proof involves recognizing that we can switch uniformily to questions 
of the second kind.
...
For instance, the squaring function $\TMfcn_{e_0}(x)=x^2$ gives this sequence
...
\end{frame}
\begin{frame}
Of course, there are countably many computable functions $\TMfcn_e$ so 
we have this table that lists every computable sequence 
...
But besides that there is something quite interesting here.
Think of the index number as the machine's name.
The machine $\TM_e$ that halts only on input~$e$ has a behavior that,
in some sense, reflects on its name.
\end{frame}
