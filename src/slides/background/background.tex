...
In this picture, the program and the number correspond.
...
\begin{frame}
  \frametitle{There exist sets that cannot be counted}
...
\begin{example}
Fix these two intervals of real numbers, 
$S=\leftclosed{0}{4}$ and $\leftclosed{1}{4}$.
Here also the injection map is one to one, so $\sizeof{S}\leq\sizeof{T}$.
\end{example}

\begin{example}
Similarly, we have $|\N|\leq |\R|$.
And, by the earlier result, the less than relation is strict.
\end{example}
\end{frame}
...
Note where the bits were flipped, in the definition of the set~$R$.
...
\item The universal machine does not get as input a Turing machine, it gets
  the index of a Turing machine, which is equivalent to
...
\begin{center}
  \parbox{0.8\textwidth}{in short:~where $e$ is the index of the 
