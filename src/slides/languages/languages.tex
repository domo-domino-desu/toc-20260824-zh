...
\begin{example}
For the alphabet of parens, $\Sigma=\set{\str{)}, \str{(}}$,
...
A Turing machine can recognize the language of the \HP,
$
  K=\set{e\in\N\suchthat \TMfcn_e(e)\converges}
$,
but no Turing machine decides~$K$.
\end{example}
\end{frame}
...
The definition of  language allows
any subset of $\kleenestar{\Sigma}$ at all.
But in practice usually there is a pattern, a list of rules.
For example, the C~language consists of programs that follow
rules of a grammar.
A natural language example is that
``When nine hundred years old you reach, look as good you will not''
may fit the rules wherever Yoda was raised but it does not fit the rules
for English.
...
\begin{frame}
We will formally write the above rules in this way.
...
The question is, given a string such as \trm{aabb}, 
...
For examples and exercises we often use uppercase letters for nonterminals
and lowercase letters or digits for terminals.
...
and then an even number of \str{b}'s (including possibly zero many \str{b}'s).
\begin{equation*}
\lang(\hat{G})=
...
\begin{grammar}
S \produces 11S \syntaxor \emptystring
\end{grammar}
...
(There are grammar types that extend the ones we consider in this course, 
for instance by allowing the head to have more than just a single 
nonterminal.
With grammars of these extended types we can compute more sets.
For instance, the set of primes requires such a grammar.) 
\end{frame}
...
\begin{frame}[fragile]
Taken from RFC~5322, 
here is a grammar for a common date and time format.
\begin{lstlisting}
...
What is the minimum number of time slots that we must use for the schedule?
\pause
\begin{center}
