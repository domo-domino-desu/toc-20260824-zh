...
\usepackage{zref-xr}
  ...
  \zexternaldocument{../../languages/languages}
...
is something that a machine can read and write as a single entity,
...
One language is
$\lang_0=\set{\sigma\in\kleenestar{\Sigma}\suchthat\text{$\sigma=\str{a}^n\str{b}\str{a}^n$ for some $n\in\N$}}$.
Note that the number of \str{a}'s at the start equals the number at the end;
some members of $\lang_0$ are 
...
Nonmembers are \str{(()}, and \str{())(()}, and \str{)(}. 
\end{example}
...
\begin{example}
There is a Turing machine that decides the language of perfect squares,
...
\begin{example}
There is a Turing machine that recognizes the set~$S$ of indices~$e$
such that $\TM_e$ halts on input~$3$.
Given~$e$, use universality to run $\TM_e$ on input~$3$
and wait for it to halt.
However, no Turing machine decides that set because
we have shown that $K$ reduces to~$S$ (so if we had a Turing machine
...
To see that there are rules consider the quote,
``When nine hundred years old you reach, look as good you will not.''
It no doubt fits the rules of wherever Yoda grew up
but object-subject-verb does not fit the rules for English.
Another example is that the C~language consists of programs that follow
rules decreed by C's standardization committee.

\pause
This is a subset of English's rules:
\begin{itemize}
...
\begin{frame}
We will write the above rules in this way.
...
\begin{remark}
This is not the most general possible definition of a grammar; 
for one thing, we could allow the head to have more than just a single 
nonterminal.
However, this form is all that we shall need,
and the grammars of mainstream programming languages largely fit this
form.
\end{remark}
\end{frame}
...
\begin{frame}
  \frametitle{Derivations}\vspace*{-1.5ex}
...
Instead, we are given a string such as \trm{aabb} and the question is 
whether we can find a terminating derivation.
\end{frame}
...
<const> ::= integer
\end{lstlisting}
But there are also a number of extensions, beyond typing convenience.
\end{frame}
