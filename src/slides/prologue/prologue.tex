...
Imagine that we get this linear system.
...
Here is a description of all such triples.
\begin{equation*}
  \set{(\frac{3}{2}-\frac{1}{2}z, \frac{1}{2}-\frac{3}{2}z, z)\suchthat z\in\R}
\end{equation*}
And, we are most charmed when we have a procedure that inputs an arbitrary
linear system and returns the complete set of solutions.
...
The approach to defining of what can be mechanically computed 
that we will follow was laid out by A~Turing. 
...
numbers.
With that mental image, he gave a number of principles that 
mechanical computation must satisfy.
...
\end{equation*}
This shows the machine starting with \str{111} on the tape 
...
\end{frame}
\begin{frame}
Our interest in the hyperoperation 
