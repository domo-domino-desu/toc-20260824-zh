...
\usepackage{zref-xr}
  ...
  \zexternaldocument{../../prologue/prologue}[../../book.pdf]
...
But better is to find all of the solution triples, as here.
\begin{equation*}
  \set{(x,y,z)=(\frac{3}{2}-\frac{1}{2}z, \frac{1}{2}-\frac{3}{2}z, z)\suchthat z\in\R}
\end{equation*}
And ideal is when we have a procedure that inputs any
...
That is, in this course we study what can be done with an algorithm.
\end{frame}


\begin{frame}{Some history}
In 1928, the well-known mathematician
...
what we today would cast into code and run on a computer.
...
heads, the number of symbols available, and the number of states, all
...
In summary, the definition is minimal,
with a computing agent that acts discretely, following a definite procedure,
and with unbounded resources, including time and also including access
to a read/write memory facility.
These are the natural choices in a mathematical abstraction of
mechanical computation. 
\end{description}
\end{frame}

\begin{frame}{An empirical question?}
Church's Thesis addresses whether Turing machines are a maximally capable
model of computation.
In a sense this is a question about
what can be done in the world.
  
As yet, there is no persuasive evidence of physically possible
systems that allow us to compute things that a Turing machine
cannot compute.
(We shall take ideas involving wormholes, black hole event horizons,
etc., 
as too speculative for this course.)
...
that we know of, the set of things computed by 
...
Church's Thesis to interpret it as \textit{No real world computer can \ldots{}}
\pause\item To prove results about Turing machines we often 
argue intuitively.
Consider the problem of whether this function is computable:~it 
inputs a number~$n$ and
returns~$1$ when $n$~is prime, and~$0$ otherwise.
...
often by giving some pseudo-code or perhaps actual code, 
...
For us, there is more danger
of being overwhelmed by the detail of four-tuples
than danger of being too hand-wavy about algorithms. 
\end{enumerate}
\end{frame}
...
\begin{frame}
\ExecuteMetaData[\catchfilefn]{ex:PrimRecFcnsi}

\pause
\ExecuteMetaData[\catchfilefn]{ex:PrimRecFcnsii}
\end{frame}
...
the Boolean function of equality between numbers,
the remainder function, the sum of a 
...
\pause
Church's Thesis is that the informal notion of `mechanically computable'
is captured by the formal one of `Turing machine computable'\Dash that the
collection of effective functions is the same as the collection of
Turing computable functions.
We gave a number of arguments in favor of this thesis.
At first glance it could appear that those same arguments 
apply to the set of primitive recursive functions.
However the above theorem shows that there are functions that
are intuitively mechanically computable but that are not primitive recursive.
So to capture `mechanically computable' with this approach
we need more than just the initial functions, composition,
and the schema of primitive recursion. 
\end{frame}
...
By the way, $g(1,1,41,0)=1$, \ldots{} $g(1,1,41,39)=1$, but
$g(1,1,41,40)=0$.
Thus $f(1,1,41)=40$.
\end{example}
\end{frame}
...
This is a suitable Boolean function.
\begin{equation*}
  g(y)=
  \begin{cases}
  ...
  \end{cases}
\end{equation*}
Goldbach's conjecture is false if this function of no arguments 
...
\pause
\ExecuteMetaData[\catchfilefn]{ex:GoldbachConjectureiii}
\end{frame}
...
Kleene proved that 
the collection of general recursive functions is the same as the
collection of Turing computable functions.

\begin{remark}
  We used Turing machines to define the set of computable functions.
  But some authors start with general recursive functions. 
  Such a development
