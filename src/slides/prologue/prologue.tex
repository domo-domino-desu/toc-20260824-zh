...
further explanation.
But what is the ``definite procedure'' about?

\pause
In Mathematics and Computer Science we take Euclid's development as a paradigm.
Theorems follow from the axioms in a prescribed way; namely,
you can verify each step of a proofa mechanically, that is, 
by pushing symbols around, without any intuition or unexplainable insights.

So, a question in the air at this time was to give a persuasive
definition of what can be computed by a machine.
\end{frame}


\begin{frame}
  \frametitle{Turing machine}
A~Turing's approach was to 
...
If instead we got a blanks, 
we would interpret that as meaning `even'.
\end{example}
\end{frame}
...
% \begin{example}
We try not to get too didactic about the representations.
Imagine that we have a Turing machine that, when given input that is a string, 
$\str{1}^n$, of $n$~many \str{1}'s, will output a string,
$\str{1}^{2n}$, of $2n$~many \str{1}'s. 
We usually won't talk about the strings but instead will feel free to
...
To show that can be computed, we will not exhibit
...
that our intuition at this is reliable.
For us, there is more of a danger
of being overwhelmed by the detail of four-tuples
than of being too vague and hand-wavy about algorithms. 
\end{enumerate}
\end{frame}
