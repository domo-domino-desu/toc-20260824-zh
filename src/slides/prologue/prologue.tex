...
\pause
To bring the question a bit more into focus:~we 
want to answer questions and solve problems.
We most like to think about questions in mathematics and
computer science, and also the natural sciences.

For instance, imagine that we get this linear system.
...
\end{array}
\end{equation*}
Finding a single $(x,y,z)$ that satisfies the system is fine
but we prefer to find them all.
This is a uniform description of all such triples.
\begin{equation*}
  \set{(\frac{3}{2}-\frac{1}{2}z, \frac{1}{2}-\frac{3}{2}z, z)\suchthat z\in\R}
\end{equation*}
And, we really are charmed when we can give a procedure that inputs an arbitrary
...
By this we mean that we want to get the answer
mechanically, by an algorithm\Dash Ouji boards don't count.
In short, 
in this course we study what can and cannot be done with a computer.
\end{frame}
...
Wanting to know what is true or false requires no
further explanation.
...
So the background is that verifying whether something is true or false 
requires what we today call an algorithm, 
which you could cast into code and run on a computer.

But in 1928 there were no computers.
Thus, a important question in the air was to give a
definition of what can be mechanically computed that is 
both correct and persuasive.
\end{frame}
...
of $2n$~many \str{1}'s. 
We usually 
say ``it computes the function $f(n)=2n$''
rather than ``from the representation of $n$ it computes  the
